...
public class Util {
	/**
	 * Performs a quicksort of the given objects by their string representation
	 * in ascending order.
	 * <p>
	 * 
	 * @param array
	...
	/**
	 * Sorts the array of objects by their string representation in ascending
	 * order.
	 * <p>
	 ...
	 *            the array of objects to sort
	 * @param start
	 *            the start index to begin sorting
	 * @param stop
	 *            the end index to stop sorting
	 * 
	...
	public static void qSortByString(Object[] array, int start, int stop) {
		...
		// arbitrarily establish a partition element as the midpoint of the
		// array
		String mid = String.valueOf(array[(start + stop) / 2]);

		// loop through the array until indices cross
		while (left <= right) {
			// find the first element that is smaller than the partition element
			// from the left
			while ((left < stop)
					&& (String.valueOf(array[left]).compareTo(mid) < 0)) {
				++left;
			}
			// find an element that is smaller than the partition element from
			// the right
			while ((right > start)
					&& (mid.compareTo(String.valueOf(array[right])) < 0)) {
				--right;
			}
		...
		// sort the left partition, if the right index has not reached the left
		// side of array
		if (start < right) {
			qSortByString(array, start, right);
		}
		// sort the right partition, if the left index has not reached the right
		// side of array
		if (left < stop) {
	 ...
	 * 
	 * @param array
	 *            the Object array to be reversed
	 */
	 ...
	 * string representation of the given object is longer then it is truncated.
	 * If it is shorter then it is padded with the blanks to the given total
	 * length. If the given object is a number then the padding is done on the
	 * left, otherwise on the right.
	 * 
	 * @param object
	 *            the object to convert
	 * @param length
	 *            the length the output string
	 */
	 ...
	 * Returns a string representation of the object in the given length. If the
	 * string representation of the given object is longer then it is truncated.
	 * If it is shorter then it is padded to the left or right with the given
	 * character to the given total length.
	 * 
	 * @param object
	 *            the object to convert
	 * @param length
	 ...
	 *            the pad character
	 * @param onLeft
	 *            if <code>true</code> pad on the left, otherwise an the right
	 */
	public static String toString(Object object, int length, char pad,
			boolean onLeft) {
